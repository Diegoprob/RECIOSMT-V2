...
const wheelLeftName = document.getElementById("wheelLeftName");
const wheelRightName = document.getElementById("wheelRightName");
...
function limpiarColorGanador() {
  ...
  wheelDuel.classList.remove("winner-left", "winner-right");
}
...
function prepararRuletaDuel(izquierda = "Jugador 1", derecha = "Jugador 2") {
  limpiarColorGanador();
  wheelLeftName.textContent = izquierda;
  wheelRightName.textContent = derecha;
  wheelCenter.textContent = "VS";
}

function marcarGanadorVisual(lado) {
  wheelCenter.textContent = "🏆";

  if (lado === "left") {
    wheelDuel.classList.add("winner-left");
  }

  if (lado === "right") {
    wheelDuel.classList.add("winner-right");
  }
}
...
function activarRuleta() {
  limpiarColorGanador();
  ruletaContainer.classList.add("ruleta-activa");
}

function detenerRuleta() {
  ruletaContainer.classList.remove("ruleta-activa");
}
...
function ruletaPro(lista, duracion = 2200) {
  return new Promise(resolve => {
    if (lista.length === 2) {
      const izquierda = lista[0];
      const derecha = lista[1];

      prepararRuletaDuel(izquierda, derecha);
      activarRuleta();

      const ganador = Math.random() < 0.5 ? izquierda : derecha;

      let alternador = 0;
      const intervaloTexto = setInterval(() => {
        ruleta.innerText = alternador % 2 === 0 ? izquierda : derecha;
        alternador++;
      }, 120);
      ...
      setTimeout(() => {
        ...
        marcarColorGanador();
        marcarGanadorVisual(ganador === izquierda ? "left" : "right");
    ...
    prepararRuletaDuel("SORTEO", "ALEATORIO");
    activarRuleta();
...
document.getElementById("btnLimpiarEquipos").addEventListener("click", () => {
  equipoA.innerHTML = "";
  equipoB.innerHTML = "";
  prepararRuletaDuel("Jugador 1", "Jugador 2");
...
document.getElementById("btnSortearMapa").addEventListener("click", async () => {
  ...
  prepararRuletaDuel("MAPA 1", "MAPA 2");
