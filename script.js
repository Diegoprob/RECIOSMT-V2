...
const wheelDuel = document.getElementById("wheelDuel");
...
function coloresRuleta(cantidad) {
  const base = [
    "#2563eb",
    "#dc2626",
    "#ff9f1c",
  ...
  return base.slice(0, Math.max(cantidad, 1));
}

function configurarRuletaVisual(opciones = ["Jugador 1", "Jugador 2"]) {
  limpiarColorGanador();
  wheelCenter.textContent = opciones.length === 2 ? "VS" : "🎲";

  const cantidad = opciones.length || 1;
  const grados = 360 / cantidad;
  const colores = coloresRuleta(cantidad);

  const partes = opciones.map((_, index) => {
    const inicio = index * grados;
    const fin = (index + 1) * grados;
    return `${colores[index % colores.length]} ${inicio}deg ${fin}deg`;
  });

  wheelDuel.style.background = `conic-gradient(from -90deg, ${partes.join(", ")})`;
  wheelLabels.innerHTML = "";

  opciones.forEach((opcion, index) => {
    const centro = index * grados + grados / 2;
    const label = document.createElement("div");
    label.className = "wheel-label";
    label.dataset.index = index;
    label.style.transform = `rotate(${centro}deg) translate(42px, -50%) rotate(${-centro}deg)`;
    label.innerHTML = `<span>${opcion}</span>`;
...
function marcarGanadorVisual(indexGanador) {
  ...
  const label = wheelLabels.querySelector(`[data-index="${indexGanador}"]`);
...
function ruletaPro(lista, duracion = 5600) {
  return new Promise(async resolve => {
    ...
    // La flecha está fija arriba. Calculamos un punto dentro del sector ganador.
    // El pequeño desplazamiento evita que parezca que siempre cae en la misma línea.
    const margenSeguro = gradosPorSector * 0.22;
    const desplazamientoAleatorio = (Math.random() * 2 - 1) * margenSeguro;
    const centroSector = indexGanador * gradosPorSector + gradosPorSector / 2 + desplazamientoAleatorio;

    const rotacionActual = ((wheelRotation % 360) + 360) % 360;
    const rotacionObjetivoModulo = (360 - centroSector) % 360;
    const diferencia = (rotacionObjetivoModulo - rotacionActual + 360) % 360;
    const vueltas = 5 + Math.floor(Math.random() * 2);

    wheelRotation += vueltas * 360 + diferencia;

    wheelDuel.style.transition = `transform ${duracion}ms cubic-bezier(0.12, 0.72, 0.10, 1)`;
    activarRuleta();
    ...
    requestAnimationFrame(() => {
      wheelDuel.style.transform = `rotate(${wheelRotation}deg)`;
    });
...
document.getElementById("btnSortearMapa").addEventListener("click", async () => {
  ...
  configurarRuletaVisual(["MAPA 1", "MAPA 2"]);
  escribirRuletaNormal("🎲 Sorteando 1er mapa...");
  await esperar(600);

  const ganador1 = await ruletaPro(copia, 5600);
  ...
  const ganador2 = await ruletaPro(copia, 5600);
...
function crearBackup() {
  return {
    ...
    version: "avanzado-mapas-bombos-1",
