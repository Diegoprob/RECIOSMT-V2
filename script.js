...
let pinValidado = sessionStorage.getItem("rakionPinAdminOk") === "true";
...
function requiereAdmin() {
  ...
  alert("Necesitas iniciar sesión como administrador y colocar el código 015.");
  return false;
}
...
function renderAdmin() {
  ...
  if (!firebaseListo) {
    ...
    estadoAdmin.className = "estado-admin bloqueado";
    estadoAdmin.textContent = `Sesión iniciada como ${usuarioActual.email}, pero falta validar PIN o ese UID no está autorizado.`;
  } else {
    estadoAdmin.className = "estado-admin bloqueado";
...
btnEntrarAdmin.addEventListener("click", async () => {
  ...
  try {
    ...
    pinValidado = true;
    sessionStorage.setItem("rakionPinAdminOk", "true");
    adminPassword.value = "";
    adminPin.value = "";
    renderAdmin();
...
btnSalirAdmin.addEventListener("click", async () => {
  pinValidado = false;
  sessionStorage.removeItem("rakionPinAdminOk");
...
document.getElementById("btnSortearMapa").addEventListener("click", async () => {
  ...
  const copia = mezclar(MAPAS_BASE);
  ...
  const mapa2 = await ruletaPro(restantes, 5700);
...
function renderResultadoMapas() {
  ...
  resultadoMapas.innerHTML = `
    <div class="mapa-ganador">🥇 Mapa 1: <strong>${mapasResultado.mapa1}</strong></div>
    <div class="mapa-ganador">🥈 Mapa 2: <strong>${mapasResultado.mapa2}</strong></div>
    <p class="ayuda">Se juega Mapa 1 dos veces y Mapa 2 dos veces.</p>
  `;
}
